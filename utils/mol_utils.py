...
def gen_mol(x, adj, node_mask,dataset, largest_connected_comp=True):
    ...
    for x_elem, adj_elem,mask in zip(x, adj,node_mask):
        mol = construct_mol(x_elem, adj_elem,mask,atomic_num_list)
...
def construct_mol(x, adj,mask, atomic_num_list): # x: 9, 5; adj: 4, 9, 9
    mol = Chem.RWMol()
